...
/** Helpers for phone-hotspot and Wi-Fi setup workflows. */
final class HotspotSupport {
    ...
    /** Best-effort discovery of a private IPv4 address reachable from local Wi-Fi clients. */
        ...
        try {
            ...
            while (interfaces != null && interfaces.hasMoreElements()) {
                ...
                while (addresses.hasMoreElements()) {
                    ...
                    Candidate candidate = new Candidate(host, scoreInterface(name, host));
    ...
    private static int scoreInterface(String name, String host) {
        int score = 0;
        if (name.startsWith("ap") || name.startsWith("swlan") || name.contains("softap")) score += 100;
        if (name.startsWith("wlan") || name.contains("wifi")) score += 60;
        if (host.endsWith(".1")) score += 40;
